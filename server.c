    ...
    char *argv[];
{
	...
	goto respond;
      ...
      retval = krb5_encrypt(context, 
			    crypto, 
			    KRB5_KU_TICKET, 
			    (void *)buf,
                            len,
			    &request.tkt.enc_part.cipher);
			 ...
			 krb525_request *request)
{
...
#ifdef HEIMDAL
    if (client.entry.pw_end && *client.entry.pw_end < now &&
	!server.entry.flags.change_pw) {
#else
     ...
     */
#ifdef HEIMDAL
    if (client.entry.flags.change_pw &&
	!server.entry.flags.change_pw) {
#else
     ...
     */
#ifdef HEIMDAL
    if (request->ticket->ticket.flags.may_postdate ||
	request->ticket->ticket.flags.postdated) {
