...
get_krb525_creds_ccache(krb5_context context,
			krb5_ccache ccache,
			char *cname, char *krb525_host, krb525_authdata * auth_data, krb5_creds ** krb525_creds)
{
	...
	if (cname == NULL) {
		...
		if ((retval = krb5_cc_get_principal(context, ccache, &in_creds.client))) {
			sprintf(krb525_convert_error, "%s while getting cache owner", error_message(retval));
			return (retval);
		}
	} else {
		/* Parse name */
		if ((retval = krb5_parse_name(context, cname, &in_creds.client))) {
			sprintf(krb525_convert_error, "%s when parsing name %s", error_message(retval), cname);
			return (retval);
		}
	}
	 ...
	 */
	if ((retval = krb5_sname_to_principal(context, krb525_host, KRB525_SERVICE, KRB5_NT_SRV_HST, &in_creds.server))) {
	...
	retval = krb5_get_credentials(context, gateway_options, ccache, &in_creds, krb525_creds);
	if (retval)
		sprintf(krb525_convert_error, "%s while getting credentials", error_message(retval));
...
krb525_do_convert(krb5_context context,
		  int sock, krb5_creds * krb525_creds, krb5_creds * in_creds, krb5_creds * out_creds)
{
	int namelen;
	...
	retval = krb5_sendauth(context, &auth_context, (krb5_pointer) & sock, KRB525_VERSION,	/* Application version */
	...
	/* Prepare to encrypt */
	if ((retval = setup_auth_context(context, auth_context, &lsin, &rsin, "_525"))) {
	...
	case STATUS_OK:
		...
		krb5_data_copy(&out_creds->second_ticket, &in_creds->second_ticket, in_creds->second_ticket.length);
...
krb525_convert_with_keytab(krb5_context context,
			   char **hosts,
			   int port, krb5_keytab keytab, char *cname, krb5_creds * in_creds, krb5_creds * out_creds)
{
...
krb525_get_creds_keytab(krb5_context context,
			krb5_keytab keytab, char *cname, krb5_creds * in_creds, krb5_creds * out_creds)
{
