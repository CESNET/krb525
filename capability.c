...
decrypt_tkt_enc_part(krb5_context context, krb5_keyblock * key, EncryptedData * enc_part, EncTicketPart * decr_part)
{
	krb5_error_code ret;
	krb5_data plain;
	size_t len;
	...
	ret = krb5_decrypt_EncryptedData(context, crypto, KRB5_KU_TICKET, enc_part, &plain);
	krb5_crypto_destroy(context, crypto);
	if (ret)
		return ret;

	ret = krb5_decode_EncTicketPart(context, plain.data, plain.length, decr_part, &len);
...
cap_check_ticket(krb525_request * request, krb5_data * cap)
{
	...
	ret =
	    _krb5_principalname2krb5_principal(request->krb5_context, &cap_ticket.server, cap_tkt.sname, cap_tkt.realm);
	if (ret)
		goto out;

	/* get server key */
	ret = hdb_get_key(request->krb5_context, cap_ticket.server, &server_key, cap_tkt.enc_part.etype);
	if (ret)
		goto out;
	...
	if (ret == KRB5KRB_AP_ERR_TKT_EXPIRED)
		ret = decrypt_tkt_enc_part(request->krb5_context, server_key, &cap_tkt.enc_part, &cap_ticket.ticket);
	if (ret)
		goto out;
	...
	if (!krb5_principal_compare(request->krb5_context, request->target_client, cap_ticket.client)) {
		cap_error = "Target client does not match capability.";
		return (-1);
	}

	if (!krb5_principal_compare(request->krb5_context, request->target_server, cap_ticket.server)) {
