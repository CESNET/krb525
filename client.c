...
get_creds_with_keytab(krb5_context context,
		      ...
		      krb5_principal server, krb5_flags options, krb5_keytab keytab, krb5_creds * creds)
{
	...
	if (retval = krb5_get_in_tkt_with_keytab(context, options, NULL, NULL, NULL, keytab, 0, creds, 0)) {
...
get_creds_with_ccache(krb5_context context,
		      krb5_principal client,
		      krb5_principal server, krb5_flags options, krb5_ccache ccache, krb5_creds * creds)
{
	krb5_error_code retval;
	...
	if (retval = krb5_get_credentials(context, 0, ccache, &in_creds, &out_creds)) {
		com_err(progname, retval, "when getting credentials");
		return retval;
	};
 ...
 */
static int
get_guid(char *username, uid_t * uid, gid_t * gid)
{
...
char *argv[];
{
	...
	if ((argc - optind) != 0)
		fprintf(stderr, "%s: Ignoring extra command line options starting with %s\n", progname, argv[optind]);

	if (keytab_name && !use_keytab) {
		fprintf(stderr, "%s: Need to specify keytab (-k) to use keytab name (-t)\n", progname);
		arg_error++;
	}

	if (use_keytab && !cname) {
		fprintf(stderr, "%s: Need to specify client name (-c) when using keytab (-k)\n", progname);
		arg_error++;
	}
	...
	if (source_cache_name)
		retval = krb5_cc_resolve(context, source_cache_name, &source_ccache);
	else
	...
	if (use_keytab) {
		if (keytab_name) {
			if (retval = krb5_kt_resolve(context, keytab_name, &keytab)) {
				com_err(progname, retval, "while parsing keytab \"%s\"", keytab_name);
				error_exit();
			}
		} else {
	...
	if (sname == NULL) {
		...
		if (retval = krb5_build_principal(context,
						  ...
						  default_realm, KRB5_TGS_NAME, default_realm, 0)) {
#else
		if (retval = krb5_build_principal(context,
						  &sprinc,
						  default_realm.length,
						  default_realm.data, KRB5_TGS_NAME, default_realm.data, 0)) {
#endif
			com_err(progname, retval, "building default service principal");
			error_exit();
		}
	} else {
		/* Service specified */
	...
	if (target_cache_name)
		retval = krb5_cc_resolve(context, target_cache_name, &target_ccache);
	else
	...
	if (use_keytab)
		retval = get_creds_with_keytab(context, cprinc, sprinc, target_options, keytab, &creds);
	else
		retval = get_creds_with_ccache(context, cprinc, sprinc, target_options, source_ccache, &creds);

	if (retval) {
		/* Detailed error message already printed */
		fprintf(stderr, "Couldn't get ticket - %s for %s", cname, sname);
		error_exit();
	}
	...
	if (!use_keytab)
		...
		retval = krb525_convert_with_keytab(context,
						    krb525_hosts, krb525_port, keytab, cname, &creds, &target_creds);
	...
	if (use_keytab ||
	    ...
	    !source_cache_name || source_cache_name && strcmp(source_cache_name, target_cache_name))
		initialize_cache = 1;
	 ...
	 */
	if (!run_aklog && !dont_run_aklog)
		krb5_appdefault_boolean(context, progname, &default_realm, "krb5_run_aklog", 0, &run_aklog);
	...
	if (run_aklog) {
		...
		krb5_appdefault_string(context, progname, &default_realm,
				       "krb5_aklog_path", INSTALLPATH "bin/aklog", &aklog_path);
		...
		if (stat(aklog_path, &st) == 0) {
			...
			if (verbose)
				printf("Can't run aklog: %s doesn't exist", aklog_path);
		}
